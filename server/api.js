...
const apiMethods = {
  ...
  get6Random: (callback) => {
    ...
    function getRandomProducts(index) {
      db.query(`SELECT * FROM products WHERE id = ${Math.floor(Math.random() * 100) + 1}`, (err, result) => {
        if (err) {
          return false;
        }
        ...
        if (usedImages.includes(jsonResult.image_url)) {
          ...
          db.query(`SELECT name FROM shops WHERE id = ${lastResult.shop_id}`, (shopErr, shopResult) => {
            if (shopErr) {
              callback(shopErr);
            } else {
